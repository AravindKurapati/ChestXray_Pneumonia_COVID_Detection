...
This was one of my first serious ML projects — built during COVID when the problem felt genuinely urgent, debugged with Stack Overflow, Kaggle forums, and a lot of trial and error. No LLMs to explain error messages. No Copilot to autocomplete the architecture. Just documentation, forum threads, and persistence.
...
In 2021, COVID-19 diagnosis was still heavily reliant on manual chest X-ray review. The idea that a CNN could flag pneumonia or COVID from a radiograph — and potentially help in under-resourced settings — felt like a real problem worth working on.
...
This project would be considered straightforward today. In 2021, getting InceptionV3 to fine-tune correctly on medical images without it immediately overfitting took real debugging.
...
This was built before those tools were accessible. The value wasn't the final accuracy. It was learning to read a loss curve, understand why a model stops learning, catch data leakage, and debug GPU memory errors from scratch.
